...
function eventReducer(events, arr) {
	if (arr[0] === "onerror" || arr[0] === "onend") ;else if (arr[0] === "ontext" && events.length && events[events.length - 1].event === "text") {
...
function getCallback(expected, done) {
	...
	return function (err, actual) {
		...
		if (repeated) done();else repeated = true;
	};
}
...
exports.mochaTest = function (name, root, test) {
	...
	function readDir(cb) {
		...
		fs.readdirSync(dir).filter(RegExp.prototype.test, /^[^\._]/) //ignore all files with a leading dot or underscore
		.map(function (name) {
